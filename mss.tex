...
and more increasingly automated, agentic pipelines for state and
...
pass \textsc{sim} costs to buyers, with accounts on Telegram and WhatsApp rising
...
predation, centralization, and enclosure \citep{Davis2022}. We instead
seek to implement a new, decentralized, sybil-resistant identity and
peer-to-peer networking stack.

This article introduces three separable but complementary layers of a
...
In Future Work \S\ref{future-work} we outline near- to
...
\sloppy
Groundwire is in part an argument about structure: we content that the
...
\citeauthor{DirectedMessaging2026}, 2026) approach to identity, attestation, and
...
policy to logical subtrees (supplementing or replacing IP ranges and
geoblocks), analyse node provenance across space (permissions,
derivative agent identities \S\ref{hierarchical-identity}) and time
(key rotations, verifiable onchain history). When this exact same
filesystem addresses data storage, \mbox{financial} transactions, and
networking traffic, the design space for simple and efficient (so,
decentralizable) sybil resistance tooling is wide open. Any subtree in
the namespace can be summed into some quantity of trust-value over and
...
preferring to leave composition of that software to the end-user.
...
The mnemonym scheme is a general-purpose scheme for generating mnemonic
...
Nostr, or Groundwire ID.

The mnemonym design is greatly inspired by \textsc{bip}-39. Like
\textsc{bip}39, a mnemonyms wordlist is an alphabetically-sorted list
...
Two key differences from \textsc{bip}-39 mnemonics obtain:

\begin{itemize}[noitemsep,topsep=2pt]

\item
  We do not expect mnemonyms wordlists to be embedded in crypto hardware
...
understand its provenance to find out it was attained and at what cost,
...
mined it on their own machine or purchased it from the miner.

Our initial English wordlist, detailed further below, consists
...
\item
  Bare nyms, e.g.,
  \begin{center}
  \texttt{abet.baboon.caffeine.denounce.escape}
  \end{center}
\item
  One-dot nyms, e.g.,
  \begin{center}
...
\item
  ...
  \texttt{..abet.baboon..denounce.escape}
  \end{center}
\item
...
Implementations must reserve these as follows:
...
\item
  ...
  receiver may or may not respect the same \textsc{pki} (e.g.~Wire \textsc{uri}s).
\end{itemize}
...
contexts where user funds may be at risk.
...
The wordlist and libraries in C, Python, and Hoon are available at
\href{https://github.com/gwbtc/mnemonyms}{\texttt{gwbtc/mnemonyms}}. Our C library is implemented in
\href{https://github.com/gwbtc/comet-miner}{\texttt{gwbtc/comet-miner}}.
...
subject to some user-defined policy that determines who can send
...
(concatenate 0 [(length-encode 'urbtc') [(length-encode 8) (length-encode (serialize pki-evidence))]])
\end{lstlisting}

Where \texttt{(concatenate\ 0\ ...)} concatenates an array of
\texttt{{[}bitwidth\ bitstring{]}} pairs according to block size
\texttt{0}. This produces a bitstring which is hashed into the pubkey to
couple it to the \textsc{pki} evidence.

\begin{lstlisting}[style=listingblock]
// pseudocode to apply the tweak to a pubkey
(sha256 pubkey (concatenate 0 [(length-encode 'urbtc') [(length-encode 8) (length-encode (serialize pki-evidence))]]))
\end{lstlisting}
...
application a node will use to verify identities on that \textsc{pki}, which may
...
of the same Groundwire ID (or, the same verified nym) appearing on two
...
\texttt{(length-encode\ (serialize\ pki-evidence))} pseudocode above
For example, in urbtc \S\ref{urbtc-pki-evidence} the \textsc{pki} evidence is the
relative ordinal number \citep{Rodarmor2022Ordinals} of a Bitcoin sat
...
// pseudocode
(concatenate 0 [(length-encode 'urbtc')
                [(length-encode 8)
                 (length-encode
...
\item
  \texttt{txid}: ID of a Bitcoin transaction whose outputs contain a
  \textsc{utxo} we are interested in.\\
\item
...
introduce a ``Suite C'' cryptographic core utilized by Groundwire
...
sends a fact to Ames along the previously mentioned subscription path,
...
keys are now out-of-date; they will not know what the new networking
...
In addition to your Azimuth sponsor---which maintains contact with
...
repos.
and \href{https://github.com/gwbtc/groundwire}{\texttt{gwbtc/groundwire}}.
...
required all nodes to index all Bitcoin blocks to find urbtc reveal
...
overwrite \textsc{pki} state. This snapshot system was just a convenience, and
...
to potential L2 architectures. So except in case of breach (see below)
...
tip of its \textsc{pki} state, even if the spend is simple custody-management
...
attestation packet and 4) continuity breach. Continuity breaches are
...
setting since our sibyl resistance mechanism \emph{is}
...
Comets introduce themselves offchain by exchanging attestation packets
...
ships, the required information is outpoints and block heights for the
...
configure their own sibyl-resistance policies including minimum
...
or tapping an NFC tag in-person.
...
\section{Wire \textsc{uri}s}\label{wire-uris}
...
\begin{lstlisting}[style=listingblock]
wire://aback.baboon.cabal.deceives.enough/
       https/groundwire.io
wire://abyss.beguiles.conceal.decrypt.engraved/
       geo/38.89278,-77.04583
\end{lstlisting}
...
An unsigned wire may be created as a human-readable reference to a
...
case if the format ever changed during a Kelvin version decrement in the
signed wire.
...
\item
  \textbf{Next 64 bytes:} 7K's EdDSA signature of a 32-byte SHA256 of
  either \texttt{key\_rotation\ \textbar{}\textbar{}\ tag\_path} or
  \texttt{key\_rotation\ \textbar{}\textbar{}\ tag\_path\ \textbar{}\textbar{}\ content\_hash},
  where \texttt{content\_hash} is itself a 32-byte hash of the octet
...
Clients can verify a signed wire with no content on receipt. If they
...
wire's \texttt{id} at the key rotation signed in the wire. Key rotation
...
\item
  ...
  supplementing trust-on-first-use for users who want to SSH into that
  server.
\item
...
\item
  ...
  its version number, so npm-style unversioned imports are impossible.
\item
  ...
  setups.
\item
  Cheap, mechanically trivial \textsc{cdn}s distribute spread load away from
  content publishers, so the cost of responding to reads trends
  asymptotically to zero. Effectful writes must be signed by the
  Groundwire ID that originally sent them via \textsc{hmac}. The likelihood that
  a write comes from a disposable ID in a botnet can be priced at the
...
\item
  Caching policies.
\item
  P2P economic incentives (congestion control, relays, \textsc{cdn}s, etc.).
\item
...
In the first instance, Groundwire's persistent cryptographic identity
across key rotations is suited to networks like Nostr, which have no
affordance for transferring identity in the event of a compromised
private key. At this point in time, key rotation is a narrow use-case
that even diligent users should rarely have to go through.

Cryptographically-relevant quantum computing makes key rotation a much
...
2\textsuperscript{5} or up to 2\textsuperscript{25}, \citet{NIST800208}) With Google recommending
...
migration of legacy wallets to PQC.
...
\textsc{bip}-360 ``Pay-to-Merkle-Root'' quantum resistance project (\citeyear{BIP0360}):
...
We have no need to specify a protocol-level solution for attenuated
permissions and access control between parent and child identities;
simply proving that one ID was derived from another gives us the
provenance we need, and gives third-party developers a base on which to
build their own permissions solutions.
...
\texttt{zuse.hoon} file which defines Urbit's kernel module APIs; the
...
\begin{quote}
If tool B sits on platform A, either both A and B must be
at absolute zero, or B must be warmer than A.
Whenever the temperature of A (the platform) declines,
the temperature of B (the tool) must also decline.
B must state the version of A it was developed against.
A, when loading B, must state its own current version, and the warmest
version of itself with which it's backward-compatible.
Of course, if B itself is a platform on which some
higher-level tool C depends, it must follow the same constraints
recursively.
\end{quote}
...
once: every Urbit release is either a new Zuse or a non-Kelvin patch to
the current distribution of Zuse. Any change to any Kelvin-versioned
file below Zuse will be slated for release in the next Zuse release.
...
onto a supermajority of nodes due to simple network effects and the
...
Technical debates amongst Bitcoin developers, such as \textsc{bip}-110 (\citeyear{BIP0110}), hold it as a
...
history is recorded, and how do we reconcile those semantics if two
consensus mechanisms disagree?
